...
> Для удобной оплаты из РФ и стабильного доступа ко всем моделям без VPN рекомендуется использовать агрегатор **GPTunnel** (`api.gptunnel.ru`). Он легко пополняется российскими картами, объединяет все популярные нейросети (GPT-4o, Claude 3.5 Sonnet, Gemini и др.) в один кабинет и предоставляет единый API-ключ.
> 🎁 **Промокод на скидку 10%:** **`ECHOSHADE10`**
> Пример конфигурации добавлен в `ai_providers.example.json`.
...
> Для удобной оплаты и стабильного доступа ко всем нейросетям без VPN рекомендуется использовать сервис **GPTunnel** (`api.gptunnel.ru`).
> 🎁 **Промокод на скидку 10%:** **`ECHOSHADE10`**
